...
目標：Web、iOS、Android 三平台體感一致、簡潔美觀，並延續 §4 平台策略（業務邏輯與 UI 分離、design tokens 集中管理）的架構要求。以下為 UX 方向指導原則，供未來畫面設計/元件開發時對照。

### 11.1 一份 Design Token，三個平台皮膚
- 顏色、字級、間距、圓角、陰影全部定義成語意化 token（例如 `color.accent`、`space.md`、`radius.card`），不得在畫面元件中寫死數值
- Web 端 Tailwind config 直接引用同一份 token；未來 React Native 版由同一份 token 生成 StyleSheet / NativeWind 設定，確保三平台視覺一致
- **原則是「調味一致，作法各異」**：三平台顏色/字級/間距永遠一致，但互動細節保留各平台原生慣例（iOS 滑動手勢與返回邏輯、Android 系統返回鍵、Web 鍵盤操作），不用同一套互動邏輯強迫三平台一致

### 11.2 底部導覽（Bottom Tab）作為跨平台共同骨架
- 主導覽固定為 Bottom Tab：**首頁（AI 建議/提醒）、聯絡人、快速記錄、AI 秘書問答、設定**
- 此模式在 iOS/Android 為原生慣例，Web 手機版（§6 明訂手機版面優先設計）亦已是慣例，三平台實作與學習成本皆低
- 快速記錄（§5.3a）建議以置中浮動按鈕（FAB）或獨立顯眼 tab 呈現，作為全 App 最低決策成本的操作入口

### 11.3 首頁：行動導向，而非靜態儀表板
- 首頁最上方呈現「今天需要處理」的 AI 建議清單（§5.6 `AgentSuggestion`），每筆卡片提供一鍵採納（不需先進入聯絡人詳情頁）
- 已逾期／未來才到期的提醒收於次要區塊，避免資訊過載，首頁只顯示今天要做的事
- 快速記錄入口需在首頁最顯眼位置（呼應 §5.3 的「不需先找到聯絡人即可記錄」原則）

### 11.4 聯絡人列表：卡片式呈現，顏色訊號取代文字判讀
- 採用卡片式列表（頭像 + 姓名 + 一行 meta：公司/職稱），不用傳統表格
- 星級（§5.2）以視覺符號呈現；超過無互動門檻（60 天，見 §5.6）的聯絡人以顏色訊號標示（例如橙色圓點），讓使用者掃視列表即可判斷誰該聯絡，不需逐筆讀取文字
- 對應 §6 效能需求（1,000 筆內需流暢），列表項元件應保持輕量、避免逐筆渲染複雜子元件；採用虛擬滾動或分頁載入

### 11.5 聯絡人詳情頁：分頁呈現，取代長捲軸
- 改為 Tab / Segmented Control 呈現「基本資料」「互動紀錄」「網路研究摘要（§5.8）」，各自 lazy load，取代舊版單頁長捲軸（舊版 `ContactDetailScreen.tsx` 738 行問題之一，見附錄）
- 呼應 §8.3 分層架構要求：畫面元件僅負責呈現與頁籤切換，資料存取與表單邏輯需拆到 `/data`、`/domain` 層

### 11.6 AI 輸出統一視覺語言（「AI 卡片」元件）
- 快速記錄確認（§5.3a）、AI 主動建議（§5.6）、話題建議（§5.5 項目 4）、問答回覆（§5.5a）四類 AI 輸出，共用同一套「AI 卡片」元件樣式：統一圖示標示「此為 AI 生成內容」、統一的確認/修改/忽略操作列
- 目的：使用者只需學習一次「AI 卡片如何操作」，即可套用到四個不同功能場景，降低認知負擔，強化「這是同一位 AI 秘書」的一致體感

### 11.7 離線與同步狀態：安靜地存在
- §5.11 offline-first 的狀態回饋（離線中／同步中／已同步）以角落小圖示呈現三態（雲朵/勾勾/旋轉圖示），不使用彈窗或警示色 banner 打斷操作
- 僅在使用者主動點擊圖示時才展開詳情說明，平時不搶注意力

### 11.8 落地優先順序（資源有限時的建議順序）
1. §11.1 Design Token 基礎設施
2. §11.2 底部導覽骨架
3. §11.6 AI 卡片元件一致性
