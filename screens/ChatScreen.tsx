...
const ChatScreen: React.FC<ChatScreenProps> = ({ user, onLogout, onNavigateHome, onNavigate }) => {
  // 模擬聯絡人數據用於展示卡片功能
  const mockFinanceContacts: Contact[] = [
    { id: '101', name: 'Alex Chen', role: 'CFO at TechCorp', avatar: 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=100&h=100&fit=crop', actionIcon: 'chevron_right' },
    { id: '102', name: 'Sarah Lee', role: 'Investment Banker', avatar: 'https://images.unsplash.com/photo-1494790108377-be9c29b29330?w=100&h=100&fit=crop', actionIcon: 'chevron_right' },
    { id: '103', name: 'Mike Ross', role: 'Financial Analyst', avatar: 'https://images.unsplash.com/photo-1500648767791-00dcc994a43e?w=100&h=100&fit=crop', actionIcon: 'chevron_right' }
  ];

  const [messages, setMessages] = useState<Message[]>([
    { 
      role: 'model', 
      text: "Hello! I'm your Networking Assistant. Ask me anything about your contacts.",
      time: '10:40 AM' 
    },
    {
      role: 'user',
      text: 'Who do I know that works in Finance?',
      time: '10:42 AM'
    },
    {
      role: 'model',
      text: 'I found 3 people in your network who work in Finance:',
      time: '10:42 AM',
      contacts: mockFinanceContacts
    }
  ]);
  ...
  const handleSend = async (textOverride?: string) => {
    ...
    try {
      // 傳遞歷史訊息給 Gemini 獲取建議
      const responseText = await getNetworkingAdvice(messages, textToSend);
            ...
            <div key={idx} className={`flex flex-col ${msg.role === 'user' ? 'items-end' : 'items-start'}`}>
              <div className={`flex items-end gap-2.5 max-w-[90%] ${msg.role === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
                ...
                <div className="flex flex-col gap-1.5">
                  <div className={`px-4 py-3 rounded-[20px] text-[15px] leading-relaxed shadow-sm ${
                    msg.role === 'user' 
        ...
        <div className="flex items-center gap-3 px-4 pb-6 pt-1">
          ...
          <button 
            ...
            className={`w-11 h-11 rounded-full flex items-center justify-center transition-all shrink-0 ${
              input.trim() ? 'bg-primary text-white shadow-lg shadow-primary/30 active:scale-95' : 'bg-slate-100 dark:bg-slate-800 text-slate-300'
            }`}
          >
